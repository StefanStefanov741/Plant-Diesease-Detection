...
def process_folder(folder_path):
    ...
    for file in files:
        ...
        if file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.JPG', '.PNG')):
            # Generate the paths for the original and noisy images
            original_image_path = os.path.join(folder_path, file)
            noisy_image_path = os.path.join(folder_path, file.replace('.', '2.'))
...
if __name__ == '__main__':
    ...
    base_path = 'C:/Users/tetij/Desktop/IVP/Plant Dataset 4GB/New Plant Diseases Dataset(Augmented)/New Plant Diseases Dataset(Augmented)/train'
